...
SRC_KEY = "taker_flow_6h"

def fetch():
    url = f"{BASE}/futures/taker-buy-sell-volume/history"
    params = {"exchange":"Binance","symbol":"BTCUSDT","interval":"6h","limit":4500}
...
def upload(data):
    path = f"{SRC_KEY}/dt={utc_today()}/part-{utc_ts()}_{hashlib.md5(str(data[0]).encode()).hexdigest()[:8]}.jsonl"
